...
def test_immediate_validation():
    ...
    with pytest.raises(ValidationError):
        settings, dynaconf = Settings(host="denied.com")
...
def test_lazy_validation_with_options():
    ...
    settings, dynaconf = Settings(
        host="denied.com", dynaconf_options=Options(trigger_validation=False)
    )
...
def test_union_enclosed_type_validates_items_operations(monkeypatch):
    class Settings(Dynaconf):
        numbers: Annotated[list[Union[int, float]], ItemsValidator(Validator(gt=3))]
...
def test_notrequired():
    ...
    with pytest.raises(ValidationError, match="person.team must is_in \['A', 'B']"):
...
def test_usage_of_dict_value():
    ...
    bruno = Person(
        ...
        numbers=[1, 2, 3],
        active=False,
        profile=Profile(username="br", kind=Kind(id=9, services=[Service(name="A")])),
    )
    jj = Person(
        name="jj",
        code=1234,
        numbers=[1, 7, 9],
        active=True,
        profile=Profile(username="j", kind=Kind(id=3, services=[Service(name="B")])),
    )
    boss = Person(
        name="Mike",
        code=45.5,
        numbers=[5, 6, 7],
        active=False,
        profile=Profile(username="m", kind=Kind(id=56, services=[Service(name="C")])),
    )
