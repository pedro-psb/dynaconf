        ...
        if namespace["__module__"] != __name__:
            ...
            for attr in attributes:
                if attr not in _annotations:
                    ...
                    raise DynaconfSchemaError(f"{attr!r} must include type annotation")
...
class DataDict(dict, metaclass=M):  # type: ignore
    ...
    __slots__ = ()
    __reserved__: dict
    __dynaconf_manager__: DynaconfManager
    _dynaconf_datadict = True  # a marker for validator_conditions.is_type_of
    __local_attributes__ = ["__reserved__", "__schema__", "__dynaconf_manager__"]
    ...
    def get_dynaconf(self):
        return DynaconfManager()

    @classmethod
    def init_dynaconf(cls, *args, **kwargs):
        dynaconf = DynaconfManager()
        settings = cls()
